...
class BaseAStarPlanner:
    ...
    def plan(self):
        ...
        open_heap = [(self.heuristic(start_idx, goal_idx), 0, start_idx)]
        came_from = {}
        ...
        while open_heap:
            ...
            for neighbor in self._get_neighbors(current):
                new_cost = cost_so_far[current] + self._move_cost(current, neighbor) + self._penalty(current, neighbor, start_idx, goal_idx)
    ...
    def _penalty(self, current, neighbor, start, goal):
        return 0 
...
# ===== 0: Basic A* ===== #
class AStarPlanner(BaseAStarPlanner):
    pass


# ===== 1: Angle ===== #
class AStarAnglePlanner(BaseAStarPlanner):
    def _penalty(self, current, neighbor, start, goal):
        p_rate = 0.3
...
# ===== 2: Distance ===== #
class AStarDistancePlanner(BaseAStarPlanner):
    def __init__(self, start_px, goal_points, grid_map):
        super().__init__(start_px, goal_points[-1], grid_map)
    ...
    def _penalty(self, current, neighbor, start, goal):
        if len(self.goal_points) == 1:
            ...
            prev_goal = self.goal_points[-2]
        curr_goal = self.goal_points[-1]
...
# ===== 3: Central ===== #
class CentralAStarPlanner(BaseAStarPlanner):
    def plan(self):
        sx, sy, valid_s = self.grid_map.get_xy_index(self.start[0], self.start[1])
        gx, gy, valid_g = self.grid_map.get_xy_index(self.goal[0], self.goal[1])
        if not valid_s or not valid_g:
        ...
        costs = self._compute_costs(successors)
        counts_from_goal = self._count_paths(goal_idx, reversed_successors, costs)
        counts_from_source = self._count_paths(start_idx, successors, costs)
        path = self._generate_path(start_idx, goal_idx, counts_from_source, counts_from_goal, successors)
    ...
    def _compute_costs(self, successors):
        costs = {}
    ...
    def _count_paths(self, source, successors, costs):
        counts = {source: 1}
        costs_map = {source: 0}
        queue = [(0, source)]
        while queue:
    ...
    def _generate_path(self, source, goal, counts_from_source, counts_from_goal, successors):
        path = [source]
        while path[-1] != goal:
            current = path[-1]
            best_succ = None
...
# ===== 4: LazyTheta* ===== #
class LazyThetaStarPlanner(BaseAStarPlanner):
    def plan(self):
        sx, sy, valid_s = self.grid_map.get_xy_index(self.start[0], self.start[1])
        gx, gy, valid_g = self.grid_map.get_xy_index(self.goal[0], self.goal[1])
        if not valid_s or not valid_g:
        ...
        while open_list:
            ...
            for neighbor in self._get_neighbors(current):
                if neighbor in visited:
                    continue
                tentative_g = g_cost.get(parent[current], float('inf')) + self._euclidean(parent[current], neighbor)
                if tentative_g < g_cost.get(neighbor, float('inf')):
                    g_cost[neighbor] = tentative_g
